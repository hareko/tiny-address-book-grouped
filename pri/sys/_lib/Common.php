...
class Common extends Base {
  ...
  public function Commands($lst)
  /*
   * form command buttons
   * in: lst -- token array
   */ {
    $cmd = $this->prop->txt->cmd;
    $htm = '';
    foreach ($lst as $c) {
      $htm .= '<button type="button" name="' . strtoupper($c) . '">' . $cmd[$c] . '</button>' . PHP_EOL;
    }
    return $htm;
  }
  
  public function SetNames($nms, $emp = 0, $sel = null)
  /*
   * form select options
   * in: nms -- names array
   *      emp -- empty option:
   ...
   */ {
    ...
    if ($emp == 1) {
      array_unshift($a, '');   /* Sunday 1st */
    } else if  ($emp == 2){
      array_push($a, '');    /* Monday 1st */
    }
    $s = ' selected="selected"';
    $c = '';
    for ($i=0; $i < count($a); $i++) {
      $cc = ((is_int($sel) && $sel == $i) || (is_string($sel) && $sel == $a[$i])) ? $s : '';
      $c .= '<option value="' . $a[$i] . '"' . $cc . '>' . $nms[$a[$i]] . '</option>' . PHP_EOL;
     }
    return $c;
  }
