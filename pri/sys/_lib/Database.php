...
//        $cc = $this->Aliases($columns, $options['_ali']);
    ...
    if (isset($options['ord'])) {
      $q .= ' ORDER BY ' . $this->Order($options['ord'], $options['_alio']);
    }
    if (isset($options['grp'])) {
      $q .= ' GROUP BY ' . $this->Order($options['grp'], $options['_alio']);
    }
  ...
  private function Order($ord, $ali)
  /*
   ...
   */ {
    ...
    foreach ($o as $key => $val) {
      $c .= ',' . $this->Aliases($key, $ali) . ' ' . ($val ? 'ASC' : 'DESC');
    }
    return substr($c, 1);
  }
   ...
   */ {
    ...
    if (¤::IsAssoc($tbs)) {
      ...
      foreach ($tbs as $key => $value) {
        ...
        $t .= $tbp . $k . ' ' . chr($cnt + 97) . ',';
        if ($value == '*') {
          $value = array_keys($this->_Fields($k));
